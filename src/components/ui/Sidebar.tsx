...
const SORT_OPTIONS: { value: SortOption; labelKey: "sidebar.newest" | "sidebar.nearest" | "sidebar.bestDeals" | "sidebar.mostTrusted" }[] = [
...
const PRICE_OPTIONS: { value: PriceRange; labelKey: "filter.priceAll" | "filter.price0to500" | "filter.price500to2000" | "filter.price2000to5000" | "filter.price5000plus" }[] = [
...
const DATE_OPTIONS: { value: DateFilter; labelKey: "filter.dateAll" | "filter.dateToday" | "filter.dateWeek" | "filter.dateMonth" }[] = [
  ...
  return (
    <>
      ...
      >
        <div
          ref={sheetRef}
          className="glass-strong rounded-t-2xl md:rounded-2xl h-[65dvh] md:h-full flex flex-col shadow-2xl overflow-hidden"
        >
          {/* Mobile drag handle */}
          <div
          ...
          {trendingOffers.length > 0 && (
            <div className="px-3.5 sm:px-4 pt-2 pb-2">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[11px] font-semibold text-orange-400 uppercase tracking-wide">{t("sidebar.trending")}</span>
              </div>
          ...
          {bestThisWeek.length > 0 && (
            <div className="px-3.5 sm:px-4 pt-2 pb-2">
              <div className="flex items-center gap-1.5 mb-2">
                <span className="text-sm">⭐</span>
                <span className="text-[11px] font-semibold text-amber-400 uppercase tracking-wide">{t("sidebar.bestThisWeek")}</span>
              </div>
              <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 snap-x">
          ...
          {/* Search */}
          <div className="px-3.5 sm:px-4 pt-2 pb-2.5">
          ...
          {/* Price range pills */}
          <div className="px-3.5 sm:px-4 pb-2 flex gap-1.5 overflow-x-auto">
            <span className="text-[10px] text-slate-600 font-medium flex-shrink-0 self-center">{t("filter.price")}:</span>
          ...
          {/* Date filter pills */}
          <div className="px-3.5 sm:px-4 pb-2 flex gap-1.5 overflow-x-auto">
            <span className="text-[10px] text-slate-600 font-medium flex-shrink-0 self-center">{t("filter.date")}:</span>
          ...
          <div className="px-3.5 sm:px-4 pb-1.5 flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              >
                ...
                <span className="hidden sm:inline">{t("sidebar.nearMe")}</span>
              </button>
              ...
              >
                <svg className="w-3 h-3" fill={showSavedOnly ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                </svg>
                {bookmarkedIds.size > 0 && (
                  <span>{bookmarkedIds.size}</span>
                )}
              </button>
          ...
          {/* Offer list */}
          <div className="flex-1 overflow-y-auto px-3.5 sm:px-4 pb-4 space-y-2">
            {offers.length === 0 ? (
      ...
      {isOpen && (
        <div
          className="md:hidden fixed inset-0 bg-black/40 z-20 animate-fade-in"
