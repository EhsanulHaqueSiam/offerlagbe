...
function StoreDetailPage() {
  ...
  useEffect(() => {
    ...
    return () => { document.title = "OfferLagbe"; };
  }, [name]);
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    >
                      {offer.logoUrl ? (
                        <img src={offer.logoUrl} alt="" className="w-full h-full rounded-xl object-cover" />
                      ) : (
                        category?.emoji ?? "🏷️"
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: category?.color ?? "#64748b" }} />
                        <span className="text-[10px] text-slate-500 font-medium">{category?.label ?? offer.category}</span>
                      </div>
